(function() {
  ...
  Game.prototype.win = function(){
    var modal = ("<div class='modal-text'><p>Поздравляем вы победили! Хотите ещё сыграть?</p><div class='modal-button'><button>Играть снова</button></div></div></div>");
    $(".modal").html(modal);
    $(".modal-button").on("click", this.newGame.bind(this));
  };

  Game.prototype.lose = function(){
    var modal = ("<div class='modal-text'><p>Игра окончена.  Ваш рекорд " + this.board.score + ". Сыграть повторно?</p><div class='modal-button'><button>Играть повторно</button></div></div></div>");
    $(".modal").html(modal);
    $(".modal-button").on("click", this.newGame.bind(this));
  };
